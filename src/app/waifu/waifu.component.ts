...
export class WaifuComponent implements OnInit {
	...
	private waifuTools: any = {
		...
		'fa-street-view': (event: any) => {
			...
			if (e && t) {
				let newTextureID = this.model.randTexture(
					parseInt(e),
					parseInt(t)
				);
		...
		'fa-camera-retro': (event: any) => {
			...
			if (e && t) {
				...
				Live2D.captureName = this.model.generateCaptureName(
					parseInt(e),
					parseInt(t)
				);
		...
		'fa-info-circle': (event: any) => {
			event.preventDefault();
			open('https://github.com/Mind1995Star/Mind1995Star.github.io/');
		},
		'fa-times': (event: any) => {
			event.preventDefault();
	...
	private windowEvent: any = {
		...
		copy: () => {
			this.r(
				'What have you copied? Remember to add the source when reprinting!',
				6e3,
				9
			);
		},
	...
	private loadWidget(): void {
		...
		for (let tool in this.waifuTools) {
			document
				...
				.addEventListener('click', this.waifuTools[tool]);
		}
		let e;
		if ('/' === location.pathname) {
			...
			e =
				t > 5 && t <= 7
					? 'Good morning! The plan for a day lies in the morning, and a beautiful day is about to begin. '
					: t > 7 && t <= 11
					? "Good morning! Work well but don't sit for a long time just get up and move around! "
					: t > 11 && t <= 13
					? "It's noon, working all morning? Now it's lunch time! "
					: t > 13 && t <= 17
					? "It's easy to get sleepy in the afternoon. Have you achieved today's sports goal? "
					: t > 17 && t <= 19
					? "It's evening! When ballet takes to the sea, even the sunset comes to take a sneak peek. "
					: t > 19 && t <= 21
					? 'Good evening, How are you doing today? '
					: t > 21 && t <= 23
					? [
							"It's already so late, rest early, good night ",
							'Take care of your eyes late at night! ',
					  ]
					: "Are you a night owl? If you haven't gone to bed so late, will you get up tomorrow? ";
			...
			e =
				location.hostname === t.hostname
					...
					: 'baidu' === o
					? `Hello! Friends from Baidu Search<br>Did you find me when searching for <span> ${
							t.search.split('&wd=')[1].split('&')[0]
					  }</span>?`
					: 'so' === o
					? `Hello! Friends from 360 search<br>Did you find me when searching for <span> ${
							t.search.split('&q=')[1].split('&')[0]
					  }</span>?`
					: 'google' === o
					? `Hello! Friends from Google search<br>Welcome to read <span>" ${
							document.title.split('-')[0]
					  }”</span>`
					: `Hello！From <span>${t.hostname}</span> friend`;
		} else {
		...
		let listener = (t: any) => {
			for (let { selector: o, text: a } of this.waifuTips.mouseover) {
				if (t.target.matches(o)) {
					a = this.getRandomValue(a).replace(
						'{text}',
						t.target.innerText
					);
		...
		for (let { date: date, text: t } of this.waifuTips.seasons) {
			...
			if (
				a.split('/')[0] <= o.getMonth() + 1 &&
				o.getMonth() + 1 <= i.split('/')[0] &&
				a.split('/')[1] <= o.getDate() &&
				o.getDate() <= i.split('/')[1]
			) {
				t = (t = this.getRandomValue(t)).replace(
					'{year}',
					o.getFullYear()
				);
				l.push(t);
